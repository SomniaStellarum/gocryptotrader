...
func (a *ANX) Run() {
	...
	forceUpdate := false
	if !common.StringDataContains(a.CurrencyPairs.Spot.Enabled.Strings(), "_") || !common.StringDataContains(a.CurrencyPairs.Spot.Available.Strings(), "_") {
	...
	for _, order := range resp {
		...
		orderDetail := exchange.OrderDetail{
			...
			CurrencyPair: currency.NewPairWithDelimiter(order.TradedCurrency,
				order.SettlementCurrency, a.CurrencyPairs.ConfigFormat.Delimiter),
			OrderDate: orderDate,
			Exchange:  a.Name,
			ID:        order.OrderID,
			OrderType: orderType,
